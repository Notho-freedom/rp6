...
class Code(tk.Tk):
    ...
    def __init__(self, root):
        ...
        #==========label==================================================================================
        title = Label(can,text=" = =  G  .  S  .  C  .  O  .  D  .  E  = = ",bd=9,relief=RIDGE,font=("Roman",50,"bold"),bg="black",fg="white")
        ...
        def supp():
            ...
            conn = sqlite3.connect("CD.db")
            cur = conn.cursor()
            cur.execute("DELETE FROM codesave WHERE CODE = ?",(code))
            conn.commit()
            conn.close()
            afficher()
        ...
        Button(en, text="act" , bg="red",fg="white",command=D_T).grid(row=0,column=17,padx=5,pady=10)
        D_T()
